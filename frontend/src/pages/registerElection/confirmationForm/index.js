...
export default class ConfirmationForm extends Component {
  ...
  factoryJson() {
    const date1 = `${this.state.calendarBegin.getFullYear()}.${this.state.calendarBegin.getMonth() +
      1}.${this.state.calendarBegin.getDate()} ${this.state.calendarBegin.getHours()}:${this.state.calendarBegin.getMinutes()}`;
    const date2 = `${this.state.calendarEnd.getFullYear()}.${this.state.calendarEnd.getMonth() +
      1}.${this.state.calendarEnd.getDate()} ${this.state.calendarEnd.getHours()}:${this.state.calendarEnd.getMinutes()}`;
  ...
  render() {
    ...
    this.cadastrarEleicao = async jsonEleicao => {
      ...
      await apiBlockchain
        .post("/createElection", jsonEleicao)
        .then(response => {
          ...
          candidates.map(string => {
            ...
            this.cadastrarCandidatos(
              factoryCandidate(name, number, electionID)
            );
          });

          this.growl.show({
            severity: "success",
            summary: "Eleição cadastrada com sucesso! Redirecionando..."
          });
          setTimeout(() => {
        ...
        .catch(error => {
          if (!error.response) {
            ...
            this.messages.show({
              ...
              summary: "Erro",
              detail:
                "Não foi possível cadastrar a eleição verifique se os dados estão corretos, ou se você tem o direito de cadastrar."
            });
            this.setState({ loading: false });
          }
        });
    ...
    this.isNumber = num => {
      //deve ter alguma função primitiva pra isso, mas como eu to sem paciencia vai assim mesmo.
      let bool = false;
      for (let i = 0; i < 10; i++) {
        if (num == i) {
          // sim a comparação deve ser feita com '==';
          bool = true;
          break;
        }
      }
    ...
    this.cadastrarCandidatos = async json => {
      const status = await apiBlockchain
        .post("/addCandidate", json)
        .catch(() => {
          this.growl.show({
            severity: "error",
            summary: "Erro ao cadastrar candidatos"
          });
        });
    };
    ...
    const dominio =
      this.state.domain === "" ? this.state.domain : "@" + this.state.domain;
    ...
    return (
      <span className="main-form">
        ...
        {candidatos.map((cand, index) => (
          <h2 key={index}>{cand.toString()}</h2>
        ))}

        {loading ? (
          <ProgressBar
            className="progressbar-confirm"
            style={{ height: 5 }}
            mode="indeterminate"
          />
        ) : (
          <button
            type="button"
            className="bt-input-confirm"
            onClick={this.confirmarCadastro}
          >
            Cadastrar
          </button>
        )}
